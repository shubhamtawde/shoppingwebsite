...
if($new == $conf_new)
{
   if($n>0)
   {
      ...
      if($curr==$row["pass"])
      {
         $upd="UPDATE user_info SET pass='$new' WHERE email='$email'";
         if($conn->query($upd) === TRUE)
         $s="Updated Password Successfully!";
         else
         $s="Some Error Occured while updating password. Please try again.";
      }
   }
}
else
{
   $s= "New Password and Confirm Password do not match! Go Back and try again!";
}
   ?>
   <!-- Header -->
